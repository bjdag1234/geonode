...
def compute_size_update(requests_query_list, area_compute=True, data_size=True, save=True):
    ...
    if len(requests_query_list) < 1:
        ...
        for r in requests_query_list:
            ...
            if data_size:
                r.juris_data_size = get_juris_data_size(
                    dissolve_shp(geometries))

            if save:
                message += settings.SITEURL + \
                    str(r.get_absolute_url().replace('//', '/')) + "\n"
                r.save()
        subject = "Area and data size computations done"
        recipient = [settings.LIPAD_SUPPORT_MAIL]
        send_mail(subject, message, settings.LIPAD_SUPPORT_MAIL,
...
def get_juris_tiles(juris_shp, user=None):
    ...
    pprint("xrange y:" + str(list(xrange(min_y + settings._TILE_SIZE,
                                         max_y + settings._TILE_SIZE, settings._TILE_SIZE))))
    ...
    for tile_y in xrange(min_y + settings._TILE_SIZE, max_y + settings._TILE_SIZE, settings._TILE_SIZE):
        for tile_x in xrange(min_x, max_x, settings._TILE_SIZE):
            ...
            tile_drp = (tile_x + settings._TILE_SIZE,
                        tile_y - settings._TILE_SIZE)
            ...
            if not tile.intersection(juris_shp).is_empty:
                gridref = 'E{0}N{1}'.format(
                    int(tile_x / settings._TILE_SIZE), int(tile_y / settings._TILE_SIZE))

                ceph_qs = CephDataObject.objects.filter(grid_ref=gridref)
                pprint("gridref:" + str(gridref) +
                       " query_length:" + str(len(ceph_qs)))
                if ceph_qs.count() > 0:
...
def get_juris_data_size(geometry):
    ...
    # Remove. Combine with get_juris_tiles
    ...
    for tile in tile_list:
        ...
        gridref = "E{0}N{1}".format(
            int(minx / settings._TILE_SIZE), int(maxy / settings._TILE_SIZE))
...
def assign_grid_ref_util(user):
    ...
    shapefile_name = UserJurisdiction.objects.get(
        user=user).jurisdiction_shapefile.name
    ...
    if geometry:
        ...
        if len(tiles) < 1:
            ...
            if len(gridref_list) == 1:
                ...
                pprint("Problematic shapefile for user {0} with shapefile {1}".format(
                    user.username, shapefile_name))
            ...
            try:
                ...
                tile_list_obj = UserTiles(
                    user=user, gridref_list=gridref_jquery)
                tile_list_obj.save()
    else:
...
def get_geometries_ogr(juris_shp_name, dest_epsg=32651):  # returns layer
    source = ogr.Open(("PG:host={0} dbname={1} user={2} password={3}".format(
        settings.DATABASE_HOST, settings.DATASTORE_DB, settings.DATABASE_USER, settings.DATABASE_PASSWORD)))
